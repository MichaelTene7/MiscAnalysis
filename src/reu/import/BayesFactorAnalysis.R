...
#this can be used to test for convergence by comparing the main combined phenotype against "phenotypes" which are subclades of the main phenotype

#what you need:
#RERs
#phenotype tree for both the combined phenotype and it's subclades
#mainTrees
...
calculateBayesFactor = function(mainTrees, RERObject, combinationTree, subclade1Name, subclade1Tree, subclade2Name, subclade2Tree){
  ...
  allBayesFactors=data.frame(matrix(nrow=nrow(RERObject), ncol=4))
  colnames(allBayesFactors)=c("Combination", subclade1Name, subclade2Name, "Minimum Ratio")
  ...
  for(i in 1:nrow(RERObject)){
    ...
    combinationSub1Ratio = allBayesFactors[i,1]/allBayesFactors[i,2]
    combinationSub2Ratio = allBayesFactors[i,1]/allBayesFactors[i,3]
    minRatio = min( combinationSub1Ratio, combinationSub2Ratio)
    allBayesFactors[i,4]=minRatio
